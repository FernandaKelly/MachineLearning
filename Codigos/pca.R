...
# Analisando a origem dos carros talvez possamos enxergar alguma 
#estrutura nova?
...
ggbiplot(mtcars.pca, obs.scale = 1, var.scale = 1,var.axes=FALSE,  
...
km = kmeans(dados[,c("hp","mpg")],2) #2 é o centro

pchs=(mtcars.country=="Japan")*1  +(mtcars.country=="Europe")*22  +
     (mtcars.country=="US")*24
...
plot(dados[,c("hp","mpg")], 
     ...
     pch=pchs,
     cex=2)
legend("topright",legend=c("Japao","Europa", "EUA"), pch=c(1,22,24), bty="n" )
...
plot(mtcars.pca$x, 
     ...
     pch=pchs,
     cex=2)
legend(x=-4 ,y=4.2, ncol=3,legend=c("Japao","Europa", "EUA"), 
       pch=c(1,22,24), bty="n", cex=1.2 )
