...
for(r in 1:rep){
  ...
  X = matrix(NA, ncol = ncol, nrow = nrow)  
  # Inicializa a matriz com dados "vazios".
  for(j in 1:ncol) X[,j] = rexp(nrow, rate = sqrt(j))      # Gera as amostras.
  ...
  eps = rexp(1)                               # Distribuição normal com n dados e desvio padrão igual a 1.
...
l2<- ggplot(dados3, aes(x=1, y=Lambda,fill = as.factor(1)))+
  geom_boxplot() +
  theme_bw() + 
  ylab(" ")+xlab("Modelo Incorreto")+theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank())+scale_y_continuous(lim = c(0, 1.5))+ theme(legend.position = "none")+scale_fill_manual(values=c("#3266A8"))
...
l3<- ggplot(dados4, aes(x=1, y=Lambda,fill = as.factor(1)))+
  ...
  theme(axis.text.x = element_blank(),axis.ticks.x = element_blank())+
  scale_y_continuous(lim = c(0, 0.1))+ theme(legend.position = "none")+
  scale_fill_manual(values=c("#5F04B4"))

figure <- ggarrange(l1,l2,ncol = 2, nrow = 1)
figure
